...
MLflow as the run parameter and the model version tag `dataset_sha256`, so it is always clear which
data a model was built from.
...
and Probability Letters*, 33 (1997), 291-297. The data is derived from a U.S. Census product and is
freely redistributable; scikit-learn ships it as one of its public example datasets.
...
are cut, to 6 decimals. Without the rounding the file is 1.9 MB, which is uncomfortably close to the
2 MB limit of the `check-added-large-files` pre-commit hook; with it the file is 1.3 MB. Six decimals
on an average room count has no effect on the model.
...
`services/training/training/preprocess.py` uses, so the rows are guaranteed to be training rows and
never test rows. A test in the training service checks this
...
table above and expect every future model version to carry the new `dataset_sha256`.
