...
**Production uses a canary rollout driven by Argo Rollouts.** The traffic share
is expressed as a share of the pods. The rollout aborts by itself when the new
version returns too many server errors.

The `Rollout` object replaces the `Deployment` in `gitops/charts/inference`.
Production runs 10 replicas and the steps are
`setWeight 10 → pause 120s → setWeight 50 → pause 120s → (100 %)`. A background
`AnalysisTemplate` asks Prometheus every 30 seconds for the share of 5xx
answers of the new model version only. The third failed measurement in a row
aborts the rollout and scales the new pods back to zero.
...
One canary pod out of eleven failed half of its requests. For the whole service
that was 2.08 % of 5xx, for about two minutes. This is the measured cost of the
strategy. A Blue-Green switch would have put the same broken version on 100 %
of the requests.
...
| Option | Why not |
|---|---|
| **Blue-Green** | Needs a second full copy of the service. Ten more pods at 256Mi would add 2.5 GiB of memory requests, on a cluster that has about 13 GiB and is already two thirds full. The switch is also all or nothing: a broken version gets 100 % of the traffic at once. |
...
| **Canary with a real traffic router** | The clean way to get an exact 10 % split, but it needs an ingress controller or a service mesh. `ingress-nginx` was archived in March 2026 and gets no fixes. The Argo Rollouts Gateway API plugin is still alpha. An ALB would cost money and put the service on the public internet, which this project avoids on purpose. |
| **Plain rolling update** | What staging uses, and it is already there. It cannot pause, cannot measure anything and cannot roll itself back, so it is not enough for production. |

Canary with replica-based weights gives the two things the assignment asks for:
a gradual 90/10 split and an automatic rollback. It needs no extra
...
- **The split is approximate.** "10 %" is one pod out of ten, not 10 % of the
  ...
  move around that number. For a demonstration this is close enough. For a
...
- **No traffic means no signal.** If nobody calls `/predict` during the canary,
  the analysis has nothing to measure. The query treats "no data" as success.
  The alternative is to abort every rollout that happens at a quiet moment,
  which is worse. The price is that a broken Prometheus looks like a healthy
  canary. The `faultRate` switch exists so the guard can be shown to work on
  purpose.
- **The registry is updated only after the canary.** The hook Job that moves the
  MLflow alias and stage is a `PostSync` hook. Argo CD runs those only when the
  Application is Healthy, which for a `Rollout` means the canary finished. So
  there is a window of a few minutes in which Git says version 4 and the
  registry still says version 3. That is the honest state, because version 3 is
  what most pods are still serving. An aborted canary leaves the registry at
  version 3 for good, which is also the truth.
- **Production is pinned twice, staging is not pinned at all.** Production
  names a version number and a SHA-256 in Git. The pod refuses to become ready
  if the file it downloads does not hash to that value. Staging follows the
  alias `staging` and reloads by itself. These are two different promises:
  staging is always newest, production is always exactly what was reviewed.
...
- **MLflow aliases and stages.** Aliases (`staging`, `production`,
  ...
  (`Staging`, `Production`, `Archived`) are set as well. The assignment
...
- **GitHub Actions is the real pipeline.** The assignment asks for GitLab CI.
  ...
  which was run locally with `gitlab-ci-local`. The same pair was accepted in
...
- **Step Functions starts a Kubernetes Job.** Training runs where the data and
  ...
  `eks:runJob.sync`, waits for the Job and brings the last log lines back. A
...
- **Loki is the source for the drift job.** The inference service already logs
  every prediction as JSON, so the drift job reads those lines back. The project
  does not need a second copy of the features in a database.

## What we learned by running it

Three things only became clear once the system was on AWS and the canary had
run for real. None of them changes the decision. All three change how the
strategy is operated, and they are in the runbook now.

- **Traffic for a canary has to come from inside the cluster.** A port-forward
  connects to one pod. The first attempt to send traffic during a canary put
  every request on a single pod, and the analysis measured nothing useful. A
  rollout at a quiet moment still passes on no data, which is accepted on
  purpose. But it means the guard is only as good as the traffic, and a demo
  without traffic proves nothing.
- **An abort is not fixed at once.** After the rollout aborted, Argo CD kept
  retrying the failed sync: five attempts, five to eight minutes. Only then did
  it look at the `git revert` that was already pushed. Production was safe the
  whole time, but "revert and it is fixed" is not instant. Pressing Terminate on
  the running operation skips the wait.
- **The replica split is approximate, and the numbers show it.** At the 10 %
  step the new pod did not get exactly a tenth of the requests. The error share
  the dashboard reported (2.08 %) is close to, but not the same as, the
  arithmetic 5 %. That is good enough to decide "abort", but not good enough to
  compare two models on quality.

## What would be done differently with more time

- A real traffic router: Gateway API, once the Argo Rollouts plugin is stable.
  The canary weight would then be a true share of the requests, and 10 replicas
  would no longer be needed to express 10 %.
- `NetworkPolicy` objects. Today any pod can reach MLflow and Postgres. Only the
  fact that nothing is public keeps that small.
...
- A managed database. Postgres is a single pod with one PVC. If the node it sits
  on goes away, MLflow is down until it is rescheduled. RDS would fix that for
  about $15 a month.
...
- Separate clusters for staging and production instead of separate namespaces.
  Namespaces are the right choice at this size, but they share a control plane,
  the nodes and the CNI.
