...
reads the `prediction` log lines of the production pods back out of Loki. It compares the eight
features with the reference sample from the training split and pushes the result to the PushGateway.
Grafana reads it from Prometheus and alerts on it.
...
**Memory: 287 MiB.** One run over the 2500 row reference sample on the development machine,
measured with `/usr/bin/time -l`. Almost all of it is importing pandas and Evidently, so the number
barely moves with the number of rows. The request is 320Mi and the limit 640Mi, which leaves room
for a full 5000 row window. The run took under 3 seconds.
...
400 rows is where that stops on this dataset. Below that the job pushes only `data_drift_samples`
and the timestamp, then exits 0. A dashboard can then tell "no traffic" apart from "the job stopped
running".

## Why the runs overlap

The schedule is every 15 minutes and the window is 60 minutes. So each run looks at data three
earlier runs have already seen. That is on purpose: a short burst of odd traffic is measured four
times instead of being missed between two windows. `concurrencyPolicy: Forbid` still makes sure two
runs never exist at the same time, because both would push the same metric under the same `job`
label.
