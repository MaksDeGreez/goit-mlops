...
`gitops/bootstrap`, which is a small Helm chart. The only output of that chart
is one `Application` object per component. That is the "app of apps" pattern:
one thing to create, and everything else follows.
...
A sync wave is the order Argo CD works in. It finishes wave 0 and waits until
those Applications are healthy before it starts wave 1. That is why PostgreSQL
is running before MLflow tries to connect. It is also why the Argo Rollouts
...
The two `t4g.large` nodes have 4 CPUs and 16 GiB in total. About 3.8 CPUs and
12 GiB of that can be given to pods. `kube-system` takes roughly another
0.7 CPU for the EKS add-ons. That still leaves spare room, and the cluster
needs it: during a canary, production runs an eleventh pod.
...
would ask for 1.3 CPU. That is a third of the cluster, for pods that answer in
about 5 ms.
...
Third party charts are used through multi-source Applications. The first source
is the chart in its own Helm repository. The second source is this repository
with `ref: values`, which lets the first source read
`$values/final-project/gitops/apps/<name>/values.yaml`. The chart is never
copied into this repository, and its version stays visible in the Argo CD UI.
...
* **Namespaces come from Terraform.** Terraform also puts the
  `mlflow-postgres` and `grafana-admin` secrets in them. So every Application
  syncs with `CreateNamespace=false`.
...
* **Deleting the root Application deletes almost everything.** Each child
  carries the finalizer `resources-finalizer.argocd.argoproj.io`. For a
  teardown, delete the root Application first and wait. Two volume claims stay
  behind, because the StatefulSet controller made them and Argo CD did not:
  `data-postgres-0` in `mlops-system` and `storage-loki-0` in `monitoring`.
  Delete those by hand, wait until `kubectl get pv` is empty, and only then let
