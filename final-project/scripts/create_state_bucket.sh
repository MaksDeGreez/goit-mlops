#!/usr/bin/env bash
#
# Creates the S3 bucket that stores the Terraform state of this project.
#
# This is step 0 of the deployment. The bucket already exists for this account
# (it was created for an earlier assignment and kept), so normally there is
# nothing to do here. Anyone starting from an empty account runs this first,
# with their own bucket name, and then changes the "bucket" line in
# terraform/stacks/*/backend.tf.
#
# The bucket is created once and kept. It is NOT managed by the Terraform code
# of the two stacks on purpose: if it were, "terraform destroy" would delete
# the bucket together with the state file it is supposed to protect.
#
