...
Inference downloads the version, hashes the same file and compares it before
deserializing (`services/inference/inference/model_store.py`,
`model_hash.py`). On a mismatch it logs `event=checksum_mismatch`, counts
`inference_model_load_failures_total{reason="checksum_mismatch"}` and keeps
`/health/ready` at 503. Kubernetes then sends it no traffic and Argo Rollouts
...
No stack trace and no echo of the input. `slowapi` limits `/predict` to
...
With 10 replicas behind one Service the real limit is ten times the configured
one, about 200 requests a second, which is what the burst above shows. A
restart forgets everything. A real deployment would keep the counter in a
shared store or in front of the service. The endpoint is not public here, which
is what keeps this acceptable.
...
role through OIDC. The trust policy allows only
`repo:MaksDeGreez/goit-mlops:ref:refs/heads/final-project`, in both the
documented and the numeric-id form of the claim. Its policy is least
privilege: push to four ECR repositories, and start or read one state machine
...
in Git holds them, and no RBAC role can read a Secret (`rbac/README.md`).
`gitleaks` runs as a pre-commit hook and as a full scan of `final-project/` and
`.github/workflows/` in CI. The state bucket is private, versioned and
encrypted, and the ECR registry host is injected at apply time, so the account
id is never written down.
...
**Controls.** Only `registry-ops` changes the registry. Every state change
...
Loki otherwise guesses one from the key `"error": null` and files a successful
promotion under "error". Alloy ships the line to Loki, where it stays
searchable and appears on the model quality dashboard. Promote, archive,
rollback and `no_change` lines were all read back out of Loki after the real
promotions. The normal path is not a command at all: production changes
because a commit changed `modelVersion`, and a `PostSync` hook Job then makes
the registry agree, with `ACTOR=argocd` and `GIT_SHA` set to the synced commit.
`delete` refuses to remove the version that is in production. In the cluster,
`mlops-engineers` have full rights in `staging` but read-only in `production`.
The only write they keep there is `patch` on `rollouts/status`, which aborts a
canary and cannot change what is deployed. `scripts/check_rbac.sh` asks the
live cluster 32 of these questions, and all 32 answered as written down.
...
image with the full commit SHA, so a tag cannot quietly change under a running
pod. The containers run as user 10001, non-root, with a read-only root
filesystem, all capabilities dropped and only `/tmp` writable.
